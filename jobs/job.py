...
df_datauniverse = spark.read.csv("hdfs:///opt/spark/data/dataset.csv", inferSchema=True, header=False)
...
df_datauniverse = df_datauniverse.toDF(*column_names)

# Preenche valores NA com a categoria "Unknown" em algumas colunas
df_datauniverse = df_datauniverse.na.fill({"workclass": "Unknown", "occupation": "Unknown", "native_country": "Unknown"})

# Drop dos valores NA
df_datauniverse = df_datauniverse.dropna()
...
dados_treino, dados_teste = df_datauniverse.randomSplit([0.7, 0.3], seed=42)
...
modelo_datauniverse = LogisticRegression(featuresCol="features", labelCol="label")
...
modelo_datauniverse_paramGrid = ParamGridBuilder().addGrid(modelo_datauniverse.regParam, [0.1, 0.01]).build()

# Cria um avaliador para modelos de classificação binária, configurado para usar a métrica AUC (Área Sob a Curva ROC)
datauniverse_evaluator = BinaryClassificationEvaluator(metricName="areaUnderROC")

# Configura o validador cruzado com o estimador (regressão logística), a grade de parâmetros, 
# o avaliador e o número de folds
datauniverse_cv = CrossValidator(estimator = modelo_datauniverse, 
                        estimatorParamMaps = modelo_datauniverse_paramGrid, 
                        evaluator = datauniverse_evaluator, 
                        numFolds = 5,
                        parallelism = 3)

# Treina o modelo de regressão logística usando validação cruzada para encontrar os melhores hiperparâmetros
modelo_datauniverse_cv = datauniverse_cv.fit(dados_treino_transformado)

# Aplica o modelo treinado ao conjunto de teste para gerar predições
modelo_datauniverse_cv_predictions = modelo_datauniverse_cv.transform(dados_teste_transformado)

# Avalia o desempenho do modelo no conjunto de teste usando a métrica AUC
auc = datauniverse_evaluator.evaluate(modelo_datauniverse_cv_predictions)

# Criando um DataFrame com a métrica AUC (Vamos salvar no HDFS no formato CSV)
from pyspark.sql import Row
modelo_datauniverse_cv_auc_df = spark.createDataFrame([Row(auc=auc)])

# Salvando no HDFS
modelo_datauniverse_cv.write().overwrite().save("hdfs:///opt/spark/data/modelo")
modelo_datauniverse_cv_auc_df.write.csv("hdfs:///opt/spark/data/auc", mode="overwrite")

print("Processamento Concluído com Sucesso. Obrigado Por Usar o Cluster da datauniverse.")